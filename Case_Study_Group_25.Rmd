...
#setwd(file.path(path, "Data/Einzelteil/"))
#getwd()
...
#setwd(path)
#getwd()
rval_part_t5 <- read_csv(file.path(path, "Data/Einzelteil/Einzelteil_T05.csv"), col_names=TRUE) %>%
...
rval_part_t6 <- read_csv(file.path(path, "Data/Einzelteil/Einzelteil_T06.csv"), col_names=TRUE) %>% 
...
rval_car_registrations <- read_csv2(file.path(path, "Data/Zulassungen/Zulassungen_alle_Fahrzeuge.csv"), col_names=TRUE) %>% 
  filter(str_detect(IDNummer, "^11-") & Zulassung >= "2010-09-21")

# Relevant file for geographical data
rval_geo_data <- read_csv2(file.path(path, "Data/Geodaten/Geodaten_Gemeinden_v1.2_2017-08-22_TrR.csv"), col_names=TRUE)
...
vehicle_data_missing_values <- head(rval_vehicle_data[is.na(rval_vehicle_data$Fehlerhaft_Datum),], 5)
component_data_missing_values <- head(rval_component_data[is.na(rval_component_data$Fehlerhaft_Datum),], 5)
